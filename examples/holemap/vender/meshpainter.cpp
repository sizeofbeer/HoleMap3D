...
namespace HoleMap {
    ...
    void MeshPainter::Setup(GLuint program)
    {
    ...
    void MeshPainter::Upload(std::filesystem::path path)
    {
        UploadFromFile(path.string());
        // VBO
        ...
        GLint normalAttribute{glGetAttribLocation(shader_program_, "inNormal")};
        ...
        GLint texCoordAttribute{glGetAttribLocation(shader_program_, "inTexCoord")};
        ...
        GLint tangentCoordAttribute{glGetAttribLocation(shader_program_, "inTangent")};
    ...
    void MeshPainter::Render(glm::mat4x4 model_matrix,
                            glm::mat4x4 view_matrix, glm::mat4x4 proj_matrix)
    {
        glUseProgram(shader_program_);

        // Get location of uniform variables
        GLint viewMatrixLoc{glGetUniformLocation(shader_program_, "viewMatrix")};
        GLint projMatrixLoc{glGetUniformLocation(shader_program_, "projMatrix")};
        GLint modelMatrixLoc{glGetUniformLocation(shader_program_, "modelMatrix")};
        GLint normalMatrixLoc{glGetUniformLocation(shader_program_, "normalMatrix")};

        GLint shininessLoc{glGetUniformLocation(shader_program_, "shininess")};

        GLint KaLoc{glGetUniformLocation(shader_program_, "Ka")};
        GLint KdLoc{glGetUniformLocation(shader_program_, "Kd")};
        GLint KsLoc{glGetUniformLocation(shader_program_, "Ks")};

          GLint IaLoc{glGetUniformLocation(shader_program_, "Ia")};
        GLint IdLoc{glGetUniformLocation(shader_program_, "Id")};
        GLint IsLoc{glGetUniformLocation(shader_program_, "Is")};

        GLint diffuseTexLoc{glGetUniformLocation(shader_program_, "diffuseTex")};
        GLint normalTexLoc{glGetUniformLocation(shader_program_, "normalTex")};
        GLint mappingModeLoc{glGetUniformLocation(shader_program_, "mappingMode")};

        // Set uniform variables used by every scene object
        glUniformMatrix4fv(viewMatrixLoc, 1, GL_FALSE, &view_matrix[0][0]);
        glUniformMatrix4fv(projMatrixLoc, 1, GL_FALSE, &proj_matrix[0][0]);
        ...
        auto modelViewMatrix{glm::mat3(view_matrix * model_matrix)};
    ...
    void MeshPainter::UploadDiffuseTexture(std::string_view path) {
    ...
    void MeshPainter::UploadNormalTexture(std::string_view path) {
        if (!std::filesystem::exists(path)) return;
    ...
    void MeshPainter::UploadFromFile(std::string_view path, bool standardize) {
        ...
        if (!materials.empty()) {
            ...
            if (!mat.diffuse_texname.empty())
                UploadDiffuseTexture(basePath + mat.diffuse_texname);

            if (!mat.normal_texname.empty()) {
                UploadNormalTexture(basePath + mat.normal_texname);
            } else if (!mat.bump_texname.empty()) {
                UploadNormalTexture(basePath + mat.bump_texname);
            }
        } else {
            // Default values
    ...
    void MeshPainter::ComputeTangents() {
