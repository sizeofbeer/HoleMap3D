...
inline void ImGui::FileBrowser::Display()
{
        ...
        reserveHeight += GetFrameHeightWithSpacing();
    {
        ...
        for(int i=0; i<fileRecords_.size(); i++)
        {
            auto &rsc = fileRecords_[i];
                          ...
                          ImGuiSelectableFlags_DontClosePopups))
            {
                ...
                const bool multiSelectAll =
                    (flags_ & ImGuiFileBrowserFlags_MultipleSelection) &&
                    IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows) &&
                    (GetIO().KeyCtrl && GetIO().KeyShift);
                if(selected)
                {
                    if(!multiSelect && !multiSelectAll)
                    {
                ...
                else if(rsc.name != "..")
                {
                    ...
                    {
                        if(multiSelectAll){
                            if (file_begin_idx_ <= i) {
                                for (int j=file_begin_idx_+1; j<i+1; j++){
                                    selectedFilenames_.insert(fileRecords_[j].name);
                                }
                            }else{
                                for (int j=i; j<file_begin_idx_; j++){
                                    selectedFilenames_.insert(fileRecords_[j].name);
                                }
                            }
                        }else if(multiSelect)
